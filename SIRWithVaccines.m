...
Yo = [8882000; initI' ;0;0];%% Initial S, I1, I2,... In, R, V
...
totalDays = 2000; %total days of program
...
for c = 1:i
    ...
    V = vertcat(V, YSol(:,n+3));
    
    %% Call the solver, concatinate new solution onto old solution 
...
h(vas+4) = plot(tSol,V, 'DisplayName', 'Vaccinated ', 'color','black');
...
N = S+ sum(I)+ R+V; %% Total Population

dSdt = -sum(beta * (S/N) .* I')+lOI*R-vaxPerDay+lOI*V; % evolution of susceptible. for seasonal add "+lOI*R" and define loI- loss of immunity rate
dIdt = beta * (S/N) .* I' - gamma .* I';% evolution of Infected populations (matrix form)
dRdt = sum(gamma .* I') -lOI*R; % Recovered for seasonal add "-lOI*R"
dVdt = vaxPerDay-lOI*V;
dYdt = [dSdt ;  dIdt; dRdt; dVdt];% Solution matrix
end
